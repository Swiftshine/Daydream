...
def build(region: str):
    ...
    compile_commands.append(f"{COMPILER} -i . -I- -i include/ -Cpp_exceptions off -enum int -Os -use_lmw_stmw on -fp hard -rostr -sdata 0 -sdata2 0 -c -o build/obj/suk_loader.o src/loader/suk_loader.cpp")

    kamek_command = f"{KAMEK} build/obj/loader.o build/obj/suk_loader.o -static=0x80001900 -output-code=build/loader.bin -output-riiv=build/SUK_{region}_loader.xml"

    # Compile code for target region
    print("Building target " + Fore.CYAN + f"{region}...\n" + Style.RESET_ALL)
    
    #f os.path.exists(f"deps/dols/SUK_{region}.dol"):
    #    kamek_command += f" -input-dol=deps/dols/SUK_{region}.dol -output-dol=build/SUK_{region}_custom.dol"
